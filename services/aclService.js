    ...
    addOrUpdateNewUser: async (req, res) => {
        ...
        try {
            ...
            if (user_id) {
                ...
                //修改权限的函数
                function updateSelectProperty(routes, id, newSelectValue) {
                    for (let route of routes) {
                        if (route.id === id) {
                            route.select = newSelectValue;
                            return;
                        }
                        if (route.children) {
                            updateSelectProperty(route.children, id, newSelectValue);
                        }
                    }
                }
                ...
                try {
                    //超级管理员权限
                    if (Math.min(...role_ids) <= 1) {
                        await connection.query(
                            `UPDATE users
                             SET routes = ?
                             WHERE user_id = ?`,
                            [JSON.stringify(routes), user_id]
                        )
                    } else if (Math.min(...role_ids) <= 2) {
                        updateSelectProperty(routes, 4, false)
                        await connection.query(
                            `UPDATE users
                             SET routes = ?
                             WHERE user_id = ?`,
                        ...
                        updateSelectProperty(routes, 1, false)
                        await connection.query(
                            `UPDATE users
                             SET routes = ?
                             WHERE user_id = ?`,
